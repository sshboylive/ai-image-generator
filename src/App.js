...
function App() {
    ...
    const [seed, setSeed] = useState(-1); // -1表示随机种子
    const [lastUsedSeed, setLastUsedSeed] = useState(null); // 添加上次使用的种子状态
                ...
                try {
                    const response = await fetch('/api/generate-image', {
                        ...
                        body: JSON.stringify({ 
                            ...
                            parameters: {
                                ...
                                width: width,
                                height: height,
                                seed: currentSeed  // 使用当前生成的随机种子，不再检查是否为-1
                            }
                        }),
                    });
                    ...
                    if (!response.ok) {
                        ...
                        if (response.status === 503 && retries < maxRetries) {
                            // 服务暂时不可用，尝试重试
                            retries++;
                            await new Promise(resolve => setTimeout(resolve, 2000 * retries)); // 等待时间随重试次数增加
                            continue;
                        }
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-6">
                ...
                {generatedImage && (
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold mb-3">生成结果</h2>
                        ...
                        <div className="mt-2 text-sm text-gray-600">
                            <div><strong>提示词:</strong> {prompt}</div>
                            {negativePrompt && <div><strong>反向提示词:</strong> {negativePrompt}</div>}
                            {lastUsedSeed !== null && (
                                <div>
                                    <strong>使用的种子值:</strong> {lastUsedSeed}
                                    <button
                                        onClick={() => setSeed(lastUsedSeed)}
